...
for page in pages:
    ...
    url = "http://diariodelsur.com.co/search/node/" + keyword + "?page=" + page
    html = get(url)
    ...
    print(url)
    articles = htmlsoup.find_all('li', class_="search-result")
    if len(articles) != 0:
        for oneArticle in articles:            
            title = oneArticle.a.text
            link = oneArticle.a['href']
            date = oneArticle.find('p', class_="search-info").text[10:-24]           
            content = ''
            html2 = ''
            print(link)
            while html2 == '':
            ...
            contenido = noodles.find_all('p', class_="rtejustify")
            if len(contenido) != 0:
                texto = ''
...
if count==10 :
    test_df.to_excel("diariodelsur_" + keyword + ".xlsx", encoding='iso-8859-1')   
    count=0
